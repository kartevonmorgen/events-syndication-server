...
final class SSNotices extends WPPluginStarter
{
  ...
  {
  	if ( !empty( $_COOKIE[ 'ss_notices' ] ) ) 
    {
      $notices = base64_decode( $_COOKIE[ 'ss_notices' ] );
  ...
  private function add( $string, $type, $static = false )
  {
    if ( is_array( $string ) ) 
    {
      ...
      foreach ( $string as $key => $string_item )
      {
        ...
        {
          if ( $this->add( $string_item, $type, $static ) === false )	
          {
            $result = false;
          }
        }
        ...
        {
          if ( $this->add_item( $string_item, $type, $static ) === false ) 	
          {
            $result = false;
          }
        }
  ...
  /* Errors */
  public function add_error( $string, $static=false ) 	
  { 
  ...
  { 
    ...
    /* Alerts */
  public function add_alert( $string, $static=false )
  { 
    return $this->add( $string, 'alerts', $static ); 
  }
  
  public function remove_alert( $key ) 
  { 
  ...
  /* Info */
  public function add_info( $string, $static=false )
  { 
    return $this->add( $string, 'infos', $static ); 
  }
  
  public function remove_info( $key ) 
  { 
  ...
  public function count_infos()
  { 
    return $this->count( 'infos' ); 
  }
    
  /* Confirms */
  public function add_confirm( $string, $static=false )
  { 
    return $this->add( $string, 'confirms', $static ); 
  }
  
  public function remove_confirm( $key )
  { 
    return $this->remove( $key, 'confirms' ); 
  }
  
  public function get_confirms()
  { 
    return $this->get( 'confirms' ); 
  }  
  
  public function count_confirms()
  { 
